...
export async function downloadCtfdData(poolId: string): Promise<void> {
    try {
        // Download the CTFD data using pool ID directly
        const response = await fetch(`${dulusBaseUrl}:${dulusPort}/ctfd/data?poolId=${poolId}`, {
            method: 'GET',
            headers: {
        ...
        a.href = url;
        a.download = `ctfd_data_${poolId}.json`;
...
export async function downloadLogins(poolId: string): Promise<void> {
    try {
        // Download the logins CSV using pool ID directly
        const response = await fetch(`${dulusBaseUrl}:${dulusPort}/ctfd/data/logins?poolId=${poolId}`, {
            method: 'GET',
            headers: {
                'Accept': 'text/plain',
                'X-API-Key': dulusApiKey
